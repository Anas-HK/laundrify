...
<div class="container-fluid">
    ...
    <div class="date-filter mb-4">
        <form action="{{ route('admin.analytics.services') }}" method="GET" class="row g-3 align-items-center">
            <div class="col-md-4">
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="metric-card">
                <div class="metric-value">{{ number_format($analytics['totalServices']) }}</div>
                <div class="metric-label">Total Services</div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="metric-card">
                <div class="metric-value">{{ number_format($analytics['newServices']) }}</div>
                <div class="metric-label">New Services</div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="metric-card">
                <div class="metric-value">{{ number_format($analytics['avgServicePrice'], 2) }} PKR</div>
            ...
            <div class="card analytics-card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-primary">
                        <i class="fas fa-chart-pie"></i> Services by Category
                    </h6>
                </div>
                <div class="card-body">
            ...
            <div class="card analytics-card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-info">
                        <i class="fas fa-chart-pie"></i> Service Price Distribution
                    </h6>
                </div>
                <div class="card-body">
                    <div class="chart-container">
            ...
            <div class="card analytics-card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-success">
                        <i class="fas fa-trophy"></i> Most Ordered Services
                    </h6>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered data-table">
                            ...
                            <tbody>
                                @foreach($analytics['mostOrderedServices'] as $service)
                                <tr>
                                    <td>{{ $service->service_name }}</td>
                                    <td>{{ App\Models\Service::find($service->id)->category ?? 'Uncategorized' }}</td>
                                    <td class="text-right">{{ number_format($service->total_ordered) }}</td>
                                    <td class="text-right">{{ number_format($service->total_ordered * (App\Models\Service::find($service->id)->service_price ?? 0)) }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const categoryLabels = Object.keys({!! json_encode($analytics['servicesByCategory']) !!});
        const categoryData = Object.values({!! json_encode($analytics['servicesByCategory']) !!});
        ...
        const priceLabels = Object.keys({!! json_encode($analytics['servicePriceDistribution']) !!});
        const priceData = Object.values({!! json_encode($analytics['servicePriceDistribution']) !!});
        ...
        new Chart(document.getElementById('priceDistributionChart'), {
            type: 'doughnut',
            data: {
                labels: priceLabels.map(label => label + ' PKR'),
                datasets: [{
                    data: priceData,
                    backgroundColor: priceColors,
                    hoverBackgroundColor: priceColors.map(color => color.replace('0.8', '1')),
                    hoverBorderColor: 'rgba(234, 236, 244, 1)',
                }]
            },
            options: {
