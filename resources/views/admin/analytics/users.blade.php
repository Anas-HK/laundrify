...
<div class="container-fluid">
    ...
    <div class="date-filter mb-4">
        <form action="{{ route('admin.analytics.users') }}" method="GET" class="row g-3 align-items-center">
            <div class="col-md-4">
    ...
    <div class="row">
        <div class="col-md-4">
            <div class="metric-card">
                <div class="metric-value">{{ number_format($analytics['totalUsers']) }}</div>
                <div class="metric-label">Total Users</div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="metric-card">
                <div class="metric-value">{{ number_format($analytics['newUsers']) }}</div>
                <div class="metric-label">New Users</div>
            </div>
        </div>
        <div class="col-md-4">
            <div class="metric-card">
                <div class="metric-value">{{ number_format($analytics['activeUsers']) }}</div>
            ...
            <div class="card analytics-card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-primary">
                        <i class="fas fa-chart-line"></i> User Registrations
                    </h6>
                </div>
                <div class="card-body">
            ...
            <div class="card analytics-card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-info">
                        <i class="fas fa-chart-pie"></i> Users by City
                    </h6>
                </div>
                <div class="card-body">
                    <div class="chart-container">
            ...
            <div class="card analytics-card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-success">
                        <i class="fas fa-chart-pie"></i> Order Frequency per User
                    </h6>
                </div>
                <div class="card-body">
                    <div class="chart-container">
                    ...
                    <p class="text-center mt-2">
                        <small class="text-muted">Average orders per user: {{ number_format($analytics['avgOrdersPerUser'], 2) }}</small>
                    </p>
                </div>
            </div>
        </div>
            ...
            <div class="card analytics-card">
                <div class="card-header">
                    <h6 class="m-0 font-weight-bold text-warning">
                        <i class="fas fa-trophy"></i> Top Users by Orders
                    </h6>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered data-table">
                            ...
                            <tbody>
                                @foreach($analytics['usersWithMostOrders'] as $user)
                                <tr>
                                    <td>{{ $user->name }}</td>
                                    <td>{{ $user->email }}</td>
                                    <td class="text-right">{{ $user->order_count }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Users by City Chart
        const cityLabels = Object.keys({!! json_encode($analytics['usersByCity']) !!});
        const cityData = Object.values({!! json_encode($analytics['usersByCity']) !!});
        ...
        const frequencyLabels = Object.keys({!! json_encode($analytics['orderFrequency']) !!});
        const frequencyData = Object.values({!! json_encode($analytics['orderFrequency']) !!});
        ...
        new Chart(document.getElementById('orderFrequencyChart'), {
            type: 'pie',
            data: {
                labels: frequencyLabels.map(label => label + ' order(s)'),
                datasets: [{
                    data: frequencyData,
                    backgroundColor: frequencyColors,
                    hoverBackgroundColor: frequencyColors.map(color => color.replace('0.8', '1')),
                    hoverBorderColor: 'rgba(234, 236, 244, 1)',
                }]
            },
            options: {
